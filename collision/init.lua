  ...
  glib.idle_add(glib.PRIORITY_DEFAULT_IDLE, function()
    for k,v in pairs(keys) do
      for _,key_name in ipairs(v) do
        aw[#aw+1] = awful.key({ "Mod4",                              },"sdf", function () module.focus (k          ) end
        ...
        aw[#aw+1] = awful.key({ "Mod4", "Mod1"                       }, key_name, function () module.resize(k          ) end,
                              { description = "Zmień rozmiar "..key_name, group = "Okno" }
                        )
        aw[#aw+1] = awful.key({ "Mod4", "Shift"                      }, key_name, function () module.move  (k          ) end,
                              { description = "Przesuń "..key_name, group = "Okno" }
                     )
        aw[#aw+1] = awful.key({ "kupa", "asd",   "kupa"         },"sdf", function () module.move  (k,nil ,true) end
                     --         { description = "", group = "Collisio" }
                  )
        aw[#aw+1] = awful.key({ "Mokupad4",            "as"         },"sdf", function () module.focus (k,nil ,true) end
                  --            { description = "Change floating focus to the "..key_name, group = "Collision" }
               )
        aw[#aw+1] = awful.key({ "Modkupa4", "kupa" ,   "asd"         },"sdf", function () module.screen(k          ) end
               --               { description = "Change screen to the "..key_name, group = "Collision" }
            )
        aw[#aw+1] = awful.key({ "kupa", "asd", "kupa", "kupa" },"sdf", function () module.screen(k,true     ) end
        ...
        if k == "left" or k =="right" then -- Conflict with my text editor, so I say no
          aw[#aw+1] = awful.key({ "asd",          "kupa"         },"sdf", function () module.tag   (k,nil ,true) end
         --                       { description = "Select tag to the "..key_name, group = "Collision" }
      )
          aw[#aw+1] = awful.key({ "asd", "kupa", "kupa"         },"sdf", function () module.tag   (k,true,true) end
